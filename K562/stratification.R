library(ggplot2)
library(vioplot)
#load("/MMCI/MS/ExpRegulation/work/data/singleCell/K562/K562_hclust.RData")
#load("/MMCI/MS/ExpRegulation/work/data/singleCell/K562/K562_hclust_transcript_5.RData")
#load("/MMCI/MS/ExpRegulation/work/data/singleCell/K562/K562_single_cell_expr_swapped.RData")
load("/MMCI/MS/ExpRegulation/work/data/singleCell/K562/K562_hclust_transcript_4.RData")
load("/MMCI/MS/ExpRegulation/work/data/singleCell/K562/K562_single_cell_expr_swapped_transcript.RData")

##################################################################################################################
##################################################################################################################

my.vioplot <- function(yalist,cols,main,names){
  plot(0,0,type="n",xlim=c(0,(length(yalist)+1)), ylim=range(yalist,na.rm=T),  xaxt = 'n', xlab ="", ylab = "",main=main)
...
k <- 4 ### With transcript clustering and 2000 set to threshold for nearby TSSs

######################################################

cell <- "K562_newBPs_transcriptClustering"


plus <- read.table("/MMCI/MS/ExpRegulation/work/data/singleCell/K562/transcript/K562_scell_plus_transcript_expression.txt",header=T,row.names = 1)
minus <- read.table("/MMCI/MS/ExpRegulation/work/data/singleCell/K562/transcript/K562_scell_minus_transcript_expression.txt",header=T,row.names = 1)
plus$transcript_id <- NULL
minus$transcript_id <- NULL

bulk_plus_expr <- log2(1 + FPKM2TPM(as.numeric(readLines("/MMCI/MS/ExpRegulation/work/data/ENC_RNAseq/K562/K562_newBPs_plus_mRNA.y"))))
bulk_minus_expr <- log2(1 + FPKM2TPM(as.numeric(readLines("/MMCI/MS/ExpRegulation/work/data/ENC_RNAseq/K562/K562_newBPs_minus_mRNA.y"))))

hm.plus <- read.table("/MMCI/MS/ExpRegulation/work/data/ENC_ChIP_HM/plus/K562/K562_newBPs_plus_newBDP.x")
hm.minus <- read.table("/MMCI/MS/ExpRegulation/work/data/ENC_ChIP_HM/plus/K562/K562_newBPs_minus_newBDP.x")
hm.nfr <- read.table("/MMCI/MS/ExpRegulation/work/data/ENC_ChIP_HM/plus/K562/K562_HM_NFR_bin_100bp.x")
...
#hm.plus <- read.table("/MMCI/MS/ExpRegulation/work/data/DEEP_ChIP_HM/01/K562/K562_plus_1bp.x")
#hm.minus <- read.table("/MMCI/MS/ExpRegulation/work/data/DEEP_ChIP_HM/01/K562/K562_minus_1bp.x")

#hm.plus <- read.table("../../DEEP_ChIP_HM/01/K562/K562_plus_10bp.x")
#hm.minus <- read.table("../../DEEP_ChIP_HM/01/K562/K562_minus_10bp.x")

dnase.plus <- as.matrix(read.table(paste("/MMCI/MS/ExpRegulation/work/data/singleCell/K562/DNase/K562_newBPs_DNase_plus_100bp.x",sep="")))
dnase.minus <- as.matrix(read.table(paste("/MMCI/MS/ExpRegulation/work/data/singleCell/K562/DNase/K562_newBPs_DNase_minus_100bp.x",sep="")))
dnase.nfr <- as.numeric(readLines("/MMCI/MS/ExpRegulation/work/data/singleCell/K562/DNase/K562_newBPs_DNase_NFR_bin.x"))
dnase.bin.cnt <- 40

tf.plus <- read.table("/MMCI/MS/ExpRegulation/work/data/ENC_ChIP_HM/TFs/K562/Selection/K562_TF_newBPs_plus_100bp.x")
tf.minus <- read.table("/MMCI/MS/ExpRegulation/work/data/ENC_ChIP_HM/TFs/K562/minus/K562_TF_newBPs_minus_100bp.x")
tf.nfr <- read.table("/MMCI/MS/ExpRegulation/work/data/ENC_ChIP_HM/TFs/K562/K562_newBPs_TF_minus_NFRplus.x")
tf.names <- readLines("/MMCI/MS/ExpRegulation/work/data/ENC_ChIP_HM/TFs/K562/K562_TF_names.txt")
...
gro.plus <- log2(1+as.numeric(readLines("/MMCI/MS/ExpRegulation/work/data/ENC_ChIP_HM/plus/K562/K562_newBPs_plus_allBDP_GRO.y")))
gro.minus <- log2(1+as.numeric(readLines("/MMCI/MS/ExpRegulation/work/data/ENC_ChIP_HM/plus/K562/K562_newBPs_minus_allBDP_GRO.y")))
##########
cage.plus <- log2(1+as.numeric(readLines("/MMCI/MS/ExpRegulation/work/data/ENC_ChIP_HM/plus/K562/K562_newBPs_plus_newBDP_CAGE.y")))
cage.minus <- log2(1+as.numeric(readLines("/MMCI/MS/ExpRegulation/work/data/ENC_ChIP_HM/plus/K562/K562_newBPs_minus_newBDP_CAGE.y")))
##########
...
#### Removing from GRO ####
gro.plus <- gro.plus[-crazy_gene_MRPL30]
gro.minus <- gro.minus[-crazy_gene_MRPL30]
#### Removing from CAGE ####
...
write.table(hm,"/MMCI/MS/ExpRegulation/work/data/singleCell/K562/K562_newBPs_HM_NFRbin_swapped.x",quote=F,row.names=F,col.names=F,sep="\t")
...
##################
temp <- gro.minus
gro.minus[swapped$swap.idx] <- gro.plus[swapped$swap.idx]
gro.plus[swapped$swap.idx] <- temp[swapped$swap.idx]
##################
### Plot single cell RNA-seq expression (heatmap) ###
...
for(i in seq(length(unique(clusts)))){
  ...
  nfr.sorted.idx <- order(nfr.coord[idx,3] - nfr.coord[idx,2],decreasing=T)
...
trans_span_coords_plus <- read.table("transcript/K562_scell_plus_transcript_coordinates.txt")
trans_span_coords_minus <- read.table("transcript/K562_scell_minus_transcript_coordinates.txt")
...
pdf("/MMCI/MS/ExpRegulation/work/data/singleCell/K562/NFR_transcriptSpan_plots_variableBins.pdf")
...
plot(x=sc,y=bulk,pch=20,xlab="avg. single cell log2 expr. (TPM)",ylab="bulk log2 expr. (TPM)",main=paste("K562 (plus & minus)\n","Spearman_cor",cors_bulkVSsingle[1],"\nPearson_cor",cors_bulkVSsingle[2]))
abline(a=0,b=1)
dev.off()
####################################################################
par(cex.main=.5)
if(F){
heatmap(as.matrix(gc.content[clustering.idx,]),Rowv=NA,Colv=NA,labRow = "",labCol = "",RowSideColors = cols,main="K562 GC-content",col=my_palette)
gc.sc.ord <- grab_grob()
#gplots::heatmap.2(as.matrix(gc.content.nfr.ordered),Rowv=F,Colv=F,labRow = "",labCol = "",dendrogram="n",trace="n",RowSideColors = cols,col = my_palette,main="K562 GC-content (NFR sorted)",key=F)
heatmap(as.matrix(gc.content.nfr.ordered),Rowv=NA,Colv=NA,labRow = "",labCol = "",main="K562 GC-content (NFR sorted)",RowSideColors = cols,col=my_palette)
gc.nfr.ord <- grab_grob()
heatmap(as.matrix(gc.content.ordered),Rowv=NA,Colv=NA,labRow = "",labCol = "",main="K562 GC-content sorted",RowSideColors = cols,col=my_palette)
gc.gc.ord <- grab_grob()
}
gplots::heatmap.2(as.matrix(gc.content[clustering.idx,]),Rowv=F,Colv=F,dendrogram="n",trace="n",labRow = "",labCol = "",RowSideColors = cols,main="K562 GC-content",col=my_palette,key=F)
gc.sc.ord <- grab_grob()
gplots::heatmap.2(as.matrix(gc.content.nfr.ordered),Rowv=F,Colv=F,labRow = "",labCol = "",dendrogram="n",trace="n",RowSideColors = cols,col = my_palette,main="K562 GC-content (NFR sorted)",key=F)
gc.nfr.ord <- grab_grob()
gplots::heatmap.2(as.matrix(gc.content.ordered),Rowv=F,Colv=F,labRow = "",labCol = "",main="K562 GC-content sorted",dendrogram="n",trace="n",RowSideColors = cols,col=my_palette,key=F)
gc.gc.ord <- grab_grob()
pdf(paste(cell,"_",k,"_GC_content_NFRbin_ordered_50bp.pdf",sep=""))
gplots::heatmap.2(as.matrix(gc.content[clustering.idx,]),Rowv=F,Colv=F,dendrogram="n",trace="n",labRow = "",labCol = "",RowSideColors = cols,main="K562 GC-content",col=my_palette,key=T,density.info="n")
gplots::heatmap.2(as.matrix(gc.content.nfr.ordered),Rowv=F,Colv=F,labRow = "",labCol = "",dendrogram="n",trace="n",RowSideColors = cols,col = my_palette,main="K562 GC-content (NFR sorted)",key=T,density.info="n")
gplots::heatmap.2(as.matrix(gc.content.ordered),Rowv=F,Colv=F,labRow = "",labCol = "",main="K562 GC-content sorted",dendrogram="n",trace="n",RowSideColors = cols,col=my_palette,key=T,density.info="n")
grid.newpage()
...
for(i in seq(k))
    for(j in seq(k))
          cors.ttest_res[i,j] <- round(wilcox.test(cors[[i]],cors[[j]],alternative="t",paired=F)$p.value,3)
...
gplots::heatmap.2(as.matrix(pm.pairs[clustering.idx,]),Rowv=F,Colv=F,labRow = "",labCol = "",RowSideColors = cols,col = my_palette,main=cell,trace="n",dendrogram="n",density.info="n")
grid.newpage()
...
upViewport(1)
my.vioplot(cors,rainbow(k),"K562 single cell Pearson Correlation",paste("clust.",seq(k)))
dev.off()
...
gplots::heatmap.2(geneProducts_mat,Rowv=F,Colv=F,dendrogram = "n",trace="n",col = my_palette,main="K562_geneProducts",notecol="black",notecex=1,cellnote=geneProducts_mat)
gplots::heatmap.2(phyper.pvals,Rowv=F,Colv=F,dendrogram = "n",trace="n",col = my_palette,main="K562_geneProducts\nhypergeo p-values",notecol="black",notecex=1,cellnote=round(phyper.pvals,2))
gplots::heatmap.2(observed_expected,Rowv=F,Colv=F,dendrogram = "n",trace="n",col = my_palette,main="K562_geneProducts\nobserved / expected",notecol="black",notecex=1,cellnote=round(observed_expected,2))
dev.off()
...
for(i in seq(length(unique(clusts)))){
  ...
  for(j in seq(length(idx))){
    if(isTRUE(all.equal(as.numeric(left[idx[j],]),as.numeric(right[idx[j],]))))
      next;
    #pheatmap(rbind(as.numeric(left[j,]),as.numeric(right[j,])),main=i,cluster_rows=F)
    p_m_data <- rbind(as.numeric(left[idx[j],]),as.numeric(right[idx[j],]))
    rownames(p_m_data) <- c(ens.bp.minus$gene[which(ens.bp.minus$ENS == rownames(minus[idx[j],]))],ens.bp.plus$gene[which(ens.bp.plus$ENS == rownames(plus[idx[j],]))])
...
plot.stratified.hm <- function(hm,clusts,k,func,func.name,hm.bin.cnt,whichHMs){
  #par(mfrow=c(3,2))
  print("in plot.stratified.hm:")
  print(dim(hm))
  cols <- rainbow(k)
  hm.heatmaps <- NULL
  hm.names <- c('H3K4me1','H3K4me3','H3K27me3','H3K36me3','H3K9me3','H3K27ac')
  if(length(whichHMs) <=3)
    par(mfrow=c(3,1))
  for(j in whichHMs){
    hm.median <- matrix(NA,nrow=k,ncol=hm.bin.cnt)
    for(kk in seq(k))
      hm.median[kk,] <- apply(hm[which(clusts==kk),seq((j-1)*hm.bin.cnt+1,j*hm.bin.cnt)],2,FUN=func)
    for(i in seq(k)){
      idx <- which(clusts==i)
      if(i == 1)
        plot(hm.median[i,],type="l",col=cols[i],ylim=range(hm.median),lwd=1,xaxt='n',bty="n",ann=F,yaxp=c(round(range(hm.median),1),1),main=hm.names[j])
      else
        points(hm.median[i,],type="l",col=cols[i],lwd=1)
    }
...
plot.stratified.dnase.heatmap <- function(dnase,clusts,k,bin.cnt){
  ...
  plot(medians[[1]],lwd=1,xaxt='n',bty="n",ann=F,ylim=range(medians),col=rainbow(k)[1],type="l",yaxp=c(round(range(medians),1),1))#,main="DNase")
  print("printing DNase median ranges:")
  print(round(range(medians),1))
  sapply(seq(2,k),function(i)lines(medians[[i]],col=rainbow(k)[i],lwd=1))
...
#pdf(paste("K562_HM_stratified_swpapped_mirrored_1bp_",k,"clusters.pdf",sep=""))
print("median")
...
print("median")
plot.stratified.hm(hm,clusts,k,median,func.name ="median",(hm.bin.cnt),c(1,6))
print("mean")
plot.stratified.hm(hm,clusts,k,mean,func.name="mean",(hm.bin.cnt),c(1,6))
dev.off()
...
if(F){
  ...
  grid.newpage()
  lay <- grid.layout(nrow = 2, ncol=7,heights=unit(c(.3,.8),rep("null",2)))#,widths=unit(rep(,8),"inches"))
  pushViewport(viewport(layout = lay))
  for(i in seq(6)){
  ...
  grid.draw(editGrob(dnase_log.grob.res$median, vp=viewport(layout.pos.row = 1,layout.pos.col = 7, clip=TRUE)))
  grid.draw(editGrob(dnase_log.grob.res$heatmap, vp=viewport(layout.pos.row = 2,layout.pos.col = 7, clip=TRUE)))
  upViewport(1)
}
##### Showing the non-logged median results
grid.newpage()
lay <- grid.layout(nrow = 2, ncol=7,heights=unit(c(.2,.8),rep("null",2)))#,widths=unit(rep(,8),"inches"))
pushViewport(viewport(layout = lay))
for(i in seq(6)){
  grid.draw(editGrob(hm_median.grobs[[i]], vp=viewport(layout.pos.row = 1,layout.pos.col = i, clip=TRUE)))
  grid.draw(editGrob(hm.grobs[[i]], vp=viewport(layout.pos.row = 2,layout.pos.col = i, clip=TRUE)))
}
grid.draw(editGrob(dnase.grob.res$median, vp=viewport(layout.pos.row = 1,layout.pos.col = 7, clip=TRUE)))
grid.draw(editGrob(dnase_log.grob.res$heatmap, vp=viewport(layout.pos.row = 2,layout.pos.col = 7, clip=TRUE)))
upViewport(1)
dev.off()
##################################################################################################################
...
my.vioplot(merged.strs,rep(rainbow(k),each=2),"K562 CAGE (minus_plus)",names=paste(rep(c("minus","plus"),times=k),rep(seq(k),each=2),sep="_"))
...
gro.cor.s <- list()
gro.cor.p <- list()
pdf(paste(cell,"_GRO_plus_minus_scatterPlots_",k,"_clusters.pdf",sep=""))
par(mfrow=c(2,2))
for(i in seq(k)){
  idx <- which(clusts==i)
  gro.str.plus[[i]] <- gro.plus[idx]
  gro.str.minus[[i]] <- gro.minus[idx]
  plot(gro.str.plus[[i]],gro.str.minus[[i]],pch=20,main="GRO K562")
  gro.cor.s[[i]] <- cor(gro.str.plus[[i]],gro.str.minus[[i]],method="spearman")
  gro.cor.p[[i]] <- cor(gro.str.plus[[i]],gro.str.minus[[i]],method="pearson")
}
dev.off()
print("GRO spearman correlation")
print(gro.cor.s[[i]])
print("GRO pearson correlation")
print(gro.cor.p[[i]])
mann_withney_res <- NULL;
for(i in seq(k))
    mann_withney_res <- c(mann_withney_res,wilcox.test(gro.str.minus[[i]],gro.str.plus[[i]],alternative="t",paired=T)$p.value)
print("GRO statistical test")
print(mann_withney_res)
writeLines(as.character(mann_withney_res),paste(cell,"_GRO_pvalues_",k,"clusters.txt",sep=""))

merged.strs <- c(rbind(gro.str.minus,gro.str.plus))
df <- data.frame(gro=c(gro.plus,gro.minus),TSS=c(rep("plus",times=length(gro.plus)),rep("minus",times=length(gro.minus))),cluster=c(clusts,clusts))
df.plus <- data.frame(gro=c(gro.plus),TSS=rep("plus",times=length(gro.plus)),cluster=clusts)
df.minus <- data.frame(gro=c(gro.minus),TSS=rep("minus",times=length(gro.plus)),cluster=clusts)
pdf(paste(cell,"_GRO_stratified_",k,"clusters.pdf",sep=""))
my.vioplot(merged.strs,rep(rainbow(k),each=2),"K562 GRO (minus_plus)",names=paste(rep(c("minus","plus"),times=k),rep(seq(k),each=2),sep="_"))
ggplot(df,aes(TSS,gro,fill=as.factor(cluster)))+geom_violin()#draw_quantiles = c(0.25, 0.5, 0.75),colour = "#3366FF")
ggplot(df.plus,aes(gro,x=TSS,fill=as.factor(cluster)))+geom_violin(draw_quantiles = c(0.25, 0.5, 0.75))
ggplot(df.minus,aes(gro,x=TSS,fill=as.factor(cluster)))+geom_violin(draw_quantiles = c(0.25, 0.5, 0.75))
boxplot(gro.str.plus,col=rainbow(k),main="plus")
boxplot(gro.str.minus,col=rainbow(k),main="minus")
dev.off()
...
pdf(paste(cell,"_single_cell_hierarchical_clustering_",k,"_correlation.pdf",sep=""))##_downsampling.pdf",sep=""))
gplots::heatmap.2(as.matrix(pm.pairs[clustering.idx,]),Rowv=F,Colv=F,dendrogram = "n",trace="n",labRow = "",labCol = "",RowSideColors = cols,col = my_palette,main=cell)
my.vioplot(cors,rainbow(k),"K562 single cell Pearson Correlation",paste("clust.",seq(k)))
dev.off()
}
#########################################################################
...
my.vioplot(nfr.sizes,rainbow(k),"K562 NFR size",names=seq(k))
boxplot(nfr.sizes,col=rainbow(k),main="K562 NFR size",names=seq(k))
dev.off()
##################################################################################################################
